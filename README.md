...
def query_ct(protocols, source, destination):
    ...
    conntrack = libnetfilterconntrack.ct_new()

    conntrack.l3proto = l3proto
    conntrack.ipv4_src = src_addr
    conntrack.ipv4_dst = dst_addr

    conntrack.l4proto = l4proto
    conntrack.port_src = src_port
    conntrack.port_dst = dst_port

    subsystem = libnetfilterconntrack.NFNL_SUBSYS_CTNETLINK_CT

    def catch_ct(nfmsgtype, conntrack):
        ct_id, = struct.unpack('!I', conntrack.id)
    ...
    handle.ct_send(libnetfilterconntrack.NFCT_Q_GET, conntrack)
...
query_ct((socket.AF_INET, socket.IPPROTO_TCP),
         ('192.168.50.40', 58198),
         ('192.168.50.50', 21))
```